...
const SolutionCard: React.FC<SolutionCardProps> = ({ title, subtitle, icon, delay }) => (
  <div 
    className="reveal-on-scroll group p-8 rounded-[32px] bg-white dark:bg-[#111827] border border-slate-100 dark:border-slate-800 shadow-[0_10px_30px_rgba(0,0,0,0.04)] dark:shadow-none hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] dark:hover:bg-slate-800 hover:-translate-y-2 transition-all duration-500"
    style={{ transitionDelay: delay }}
  >
    <div className="w-16 h-16 rounded-2xl bg-[#F0F9FF] dark:bg-slate-800 flex items-center justify-center text-3xl mb-6 group-hover:scale-110 transition-transform duration-500 border border-blue-50 dark:border-slate-700">
      {icon}
    </div>
    <h3 className="font-serif text-2xl text-[#0A3A78] dark:text-white mb-3 font-bold">{title}</h3>
    {subtitle && <p className="text-[#64748B] dark:text-slate-400 leading-relaxed text-[0.95rem]">{subtitle}</p>}
  </div>
);
