...
export const PaymentOutcomeChoice: React.FC = () => {
    ...
    const navigate = (path: string) => {
        // Append returnUrl if it exists
        const url = `${path}${path.includes('?') ? '&' : '?'}returnUrl=${encodeURIComponent(returnUrl)}`;
            ...
            <div style={styles.card}>
                ...
                <div style={styles.actions}>
                    ...
                    <button
                        ...
                        onClick={() => navigate(`#/payment-failure?error=Demo%20failure&planName=${encodeURIComponent(params.get('planName') || '')}&price=${encodeURIComponent(params.get('price') || '')}`)}
                    >
                        Failure
                    </button>
                </div>
