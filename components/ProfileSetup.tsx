...
export const ProfileSetup: React.FC = () => {
  ...
  const handleSubmit = () => {
    ...
    // Navigate to "Choose Plan" page instead of Assessment directly
    window.location.hash = `#/${formData.language}/choose-plan`;
  };
  ...
  // Shared Input Styles
  const inputClass = "w-full px-5 py-3 rounded-2xl border outline-none transition-all duration-300 font-medium text-[0.95rem] " +
  ...
  return (
    <div className="min-h-screen bg-[#F0F9FF] dark:bg-[#020617] flex flex-col items-center justify-center px-4 py-8 transition-colors duration-500 relative">
      ...
      {/* Main Card */}
      <div className="w-full max-w-[480px] bg-white dark:bg-[#1E293B] rounded-[32px] p-6 md:p-8 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] border border-slate-100 dark:border-slate-800 transition-colors mt-12 md:mt-0">

        <h1 className="text-[2rem] font-serif font-bold text-[#0A3A78] dark:text-white mb-2">Profile Setup</h1>

        <p className="text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4">Step 1 of 3 — Basic Info</p>
        ...
        <div className="mt-6">
          <button
            ...
            className="w-full py-3.5 rounded-full bg-gradient-to-r from-[#0052CC] to-[#2684FF] text-white font-bold text-lg hover:shadow-lg hover:brightness-105 active:scale-95 transition-all shadow-md"
          >
            Next → Wellness Check
          </button>
        </div>
        ...
        {/* DPDPA Note */}
        <div className="mt-4 bg-purple-50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-800 rounded-2xl p-3 flex items-center gap-4">
          <div className="text-2xl">🔒</div>
          <div>
            <h4 className="text-[#8B5CF6] font-bold text-sm">DPDPA Compliant</h4>
            <p className="text-slate-500 dark:text-slate-400 text-xs">Your data stays in India. You own it.</p>
          </div>
        </div>
