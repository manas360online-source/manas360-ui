      ...
      <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-20 overflow-x-hidden">
        ...
        <div className="bg-gradient-to-br from-[#667eea] to-[#764ba2] px-6 py-20 text-center relative overflow-hidden shadow-2xl">
          ...
          <div className="absolute top-6 left-6 z-30">
            <button
              onClick={onBack}
              className="flex items-center gap-2 text-white/70 hover:text-white transition-all font-bold text-xs uppercase tracking-widest bg-white/10 hover:bg-white/20 px-4 py-2 rounded-full backdrop-blur-md border border-white/10"
            >
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between border border-slate-100 gap-8">
            ...
            <div className="flex gap-3 flex-1 max-w-md">
              {TRAINING_LANGUAGES.map((l) => (
                <div
                  key={l.code}
                  className={`flex-1 h-2.5 rounded-full transition-all duration-700 shadow-sm ${isModuleComplete(l) ? 'bg-gradient-to-r from-[#667eea] to-[#764ba2]' : 'bg-slate-100'
                    }`}
                  title={l.label}
                />
              ))}
