...
interface ModuleViewerProps {
  ...
  onComplete: (code: string, score: number) => void;
}

const ModuleViewer: React.FC<ModuleViewerProps> = ({ progress, onComplete }) => {
  ...
  const currentCode = code || 'ATMT_0.1';
  ...
  const submitQuiz = () => {
    ...
    if (percentage >= module.passing_score) {
      ...
      onComplete(module.code, percentage);
    }
  };
        ...
        <div className="max-w-6xl mx-auto px-6 py-16">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12">
            ...
            <div className="flex items-center space-x-2 bg-stone-50 p-2 rounded-full border border-stone-100 shrink-0">
              ...
              <button
                onClick={() => !isPreviewMode && setActiveTab('quiz')}
                disabled={isPreviewMode}
                className={`px-8 py-4 rounded-full font-bold text-sm transition-all flex items-center ${activeTab === 'quiz' ? 'bg-white text-slate-900 shadow-xl border border-blue-50' : isPreviewMode ? 'text-stone-300 cursor-not-allowed' : 'text-slate-400'
                  }`}
              >
                Assessment
              </button>
              ...
              <div className="bg-white rounded-[4rem] p-12 md:p-20 shadow-2xl border border-white">
                <div className="prose prose-stone max-w-none">
                  ...
                  <div className="space-y-6">
                    <h3 className="text-2xl font-serif font-bold text-slate-800">Knowledge Pillars</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {['Core Methodology', 'Clinical Context', 'Patient Ethics', 'Transformation Triggers'].map((pillar, i) => (
                        <div key={i} className="flex items-center p-8 bg-stone-50/50 rounded-[2.5rem] border border-stone-100 group hover:border-blue-200 transition-colors">
                          <div className="w-12 h-12 rounded-2xl bg-white text-[#0066FF] flex items-center justify-center shadow-sm mr-6 shrink-0 group-hover:bg-[#0066FF] group-hover:text-white transition-all">
                            <CheckCircle className="w-6 h-6" />
                          </div>
                          <span className="text-slate-700 font-bold text-sm tracking-tight">{pillar}</span>
                        </div>
                      ))}
                    </div>
                  </div>
